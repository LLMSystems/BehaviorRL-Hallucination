...
class LLMChat:
    ...
    def chat(self, query, history=[], system='你是一個專業的助手，會用繁體中文回答問題。', params=None, response_format=None, stream=False, extra_body=None, multi_response=False, include_reasoning=False):
        if multi_response:
        ...
        response = self._handle_response(completion, multi_response=multi_response, include_reasoning=include_reasoning)
        
        return response, history

    def _handle_response(self, completion, multi_response=False, include_reasoning=False):  
        if multi_response:
            responses = []
